...
## 2026-08-30 — item 63: auditoria pré-release, Segurança (S2/S4/S5/S7/S9) fechada

Os 24 achados restantes da auditoria do item 58 (37 achados originais, 5 já
resolvidos pelo card 96) ganharam registro próprio no item 63. As 5 falhas
de segurança foram implementadas e verificadas ao vivo nesta rodada:

- **S5** — `sandbox.ts` vazava todo `$HOME` (`~/.ssh`, a própria
  `secrets.json`) pro comando aprovado dentro do bwrap; `--tmpfs $HOME`
  antes do `--bind root root` oclui e re-monta só o necessário. Confirmado
  ao vivo com invocação real de `bwrap`: antes vazava os dois, depois
  nenhum, escrita na raiz do projeto intacta.
- **S9** — `secrets.json` sem escrita atômica; `writeAll` agora escreve
  em `.tmp` + `renameSync`, `chmodSync` explícito (mesmo padrão aplicado
  também a `remote-devices.json` novo do S7).
- **S4** — profundidade de spawn era autodeclarada pelo cliente
  (`depth: 3` falso nunca era barrado por quem realmente estava em
  depth 0). `cardSpawnDepth` (`message-bus.ts`) vira o registro
  server-side real, indexado por cardId; `depth` sumiu do schema MCP e
  do `acbridge` (campo morto, removido, não só ignorado). Dois smoke
  tests reescritos (`smoke-mcp.mjs`, `smoke-mcp-autonomous-mode.mjs`)
  pra montar uma cadeia real de spawns em vez de confiar num
  `depth` forjado — achado ao vivo no processo: o teto de concorrência
  do modo autônomo vale pra QUALQUER novo spawn (mesmo bash), não só
  pros que contam pro teto, travando um dos dois testes até isso ficar
  claro.
- **S7** — porta do servidor remoto abria no boot, sem `Origin` no
  upgrade do WS, token na própria URL. `remote-server.ts`: bind sob
  demanda (`ensureListening`, só no primeiro `pairNewDevice`), checagem
  de mesma origem contra o `Host` da própria requisição (não uma
  allowlist de IP de LAN fixa — funciona também pelo túnel do Fase B),
  token movido pra primeira mensagem do socket. `devices` agora
  persiste via `safeStorage` (mesmo padrão de `secrets.ts`), sobrevive a
  restart — precisou de um ajuste no próprio harness de teste
  (`cdp-client.mjs` ganhou `preserveUserData`, já que `startApp` sempre
  limpava o `userDataDir` entre chamadas, o que também apagava o
  arquivo de devices que o teste de restart precisava preservar).
- **S2** — `getDisplayMedia` aprovava `sources[0]` sem perguntar;
  câmera/mic/geolocalização caíam no default permissivo do Electron.
  Achado ao vivo corrigindo a premissa original: nesta versão do
  Electron (42), `getDisplayMedia()` chega em
  `setPermissionRequestHandler` como um `"media"` genérico, idêntico ao
  de `getUserMedia()` — não dá pra distinguir os dois nessa camada, daí
  os DOIS modais em cadeia (genérico, depois específico nomeando a
  página) em vez de um só.
- Verificado ao vivo, sem mock, em todos os 5: novos
  `smoke-remote-control.mjs` (rescrito, 19 checks) e
  `smoke-browser-display-media-gate.mjs` (10 checks); regressão limpa em
  `smoke-mcp.mjs`, `smoke-mcp-spawn-queue.mjs`, `smoke-browser.mjs`.
  `tsc --noEmit`/`electron-vite build` limpos em cada passo.
- Detalhe completo (fix aplicado + critério de verificação por item) em
  `DESIGN-BACKLOG.md` item 63, seção Segurança.

## 2026-08-30 — item 64: scrollbar do terminal escondida + roda do mouse invertida

Reportado ao vivo pelo usuário. Dois fixes em `cards.css`/`useTerminal.ts`:

- Scrollbar do xterm agora genuinamente `visibility: hidden` (não só
  transparente como o item 57 ponto 1 já tinha deixado) — nos dois
  containers da scrollbar, não no ancestral que também carrega o canvas
  real do terminal.
- Achado ao vivo lendo o código vendorizado do xterm
  (`@xterm/xterm/src/vs/base/browser/mouseEvent.ts`): `StandardWheelEvent`
  negava `deltaY` sozinho (herdado do VS Code), resultando invertido
  no Electron+Wayland deste app. `correctZoomCoords` (mesmo
  interceptador de capture-phase que já corrige clique/drag sob zoom)
  agora também intercepta `wheel` SEMPRE (mesmo em zoom 1) e nega o
  `deltaY` de novo, cancelando a inversão antes do xterm ver o evento.
- **Limitação honesta**: o CDP `Input.dispatchMouseEvent(mouseWheel)`
  usado pra verificar mostrou direção correta tanto antes quanto depois
  do fix (não reproduz a inversão relatada) — confirmado que o
  interceptador de fato dispara (spy no listener), mas só o usuário
  testando em hardware real fecha esse loop. `tsc --noEmit`/
  `electron-vite build` limpos; `smoke-card-wheel-scope.mjs` (6/6) sem
  regressão.
- Detalhe completo em `DESIGN-BACKLOG.md` item 64.

## 2026-08-30 — item 63: auditoria pré-release, B1/B2/B3 fechados

Continuando o item 63 (usuário pediu pra seguir enquanto não conseguia
testar o item 64 ao vivo). Três bugs de lógica implementados e
verificados ao vivo:

- **B1** — diff de consentimento mentia como "arquivo novo" quando o
  arquivo existente passava de 512KB (`buildWriteConsent` não tratava
  `{tooLarge: true}`, `oldContent` caía pra `""`). Agora recusa a
  escrita direto, sem modal nenhum — mesma postura já usada pelo `bash`
  quando o sandbox está indisponível: nada seguro pro humano aprovar
  sem conseguir mostrar um diff honesto.
- **B2** — consentimento pendente (`pendingWriteConsents`/
  `pendingBashConsents`) não tinha dono nem prazo: fechar o card que
  pediu ou recarregar a janela deixava o loop de tools do provider
  pendurado pra sempre, já que nenhum humano pode mais clicar num botão
  que não existe. Não virou um timeout cego (deliberação humana não
  devia ser punida por tempo) — os dois mapas passaram a guardar
  `{cardId, resolve}`, e um novo `resolveConsentsForCard(cardId | null)`
  nega (e remove) o pendente certo quando um card fecha de verdade
  (`chat:card-closed`, disparado por `finalizeCloseCard`) ou nega TODOS
  quando a janela navega/recarrega (`did-start-navigation` no frame
  principal).
- **B3** — parser do socket do `acbridge` descartava tudo depois do
  primeiro `\n` de um chunk TCP (`buf = ""` em vez de fatiar o resto).
  Virou um loop coletando todas as linhas completas do chunk, cada uma
  despachada via `Promise.all`, resposta simétrica (uma linha JSON por
  linha de request).
- Verificado ao vivo, sem mock: novos
  `smoke-chat-write-consent-large-file.mjs` (4 checks),
  `smoke-chat-consent-teardown.mjs` (11 checks — fechar card real nega
  um `write_file` pendente sem travar, `Page.reload` real via CDP nega
  um `bash` pendente), `smoke-acbridge-socket-framing.mjs` (3 checks —
  duas linhas JSON no mesmo `socket.write()`, ambas processadas).
  Regressão limpa em `smoke-chat-tools.mjs`/`smoke-acbridge.mjs`.
  `tsc --noEmit`/`electron-vite build` limpos em cada passo.
- Detalhe completo (fix aplicado + verificação ao vivo, incluindo um bug
  de teste real achado e corrigido no meio do caminho) em
  `DESIGN-BACKLOG.md` item 63, seção Bugs de lógica.

## 2026-08-30 — item 63: auditoria pré-release, B6 fechado

- **B6** — listener de `ipcMain` (`snapshot:rect-reply`/`readcard:reply`)
  vazava quando o renderer nunca respondia (janela travada): o timeout de
  10s do `message-bus` resolvia a Promise pro chamador, mas nada avisava
  o main pra remover o listener que ficava esperando uma resposta que
  nunca viria. Dois novos callbacks (`onSnapshotTimeout`/
  `onReadCardTimeout`) disparados de dentro do próprio `setTimeout` do
  message-bus chamam de volta pra `main/index.ts`, que guarda a MESMA
  função de cleanup (`ipcMain.removeListener`, já existia dentro de
  `onReply`) num `Map<requestId, () => void>` novo por canal — zero
  lógica de cleanup duplicada, só mais um caminho até ela.
- Verificado ao vivo, sem mock: novo `smoke-listener-leak-timeout.mjs`
  (11 checks) — simula "renderer nunca responde" navegando a própria
  janela pra `about:blank` via CDP `Page.navigate` real (não um
  monkey-patch: uma tentativa inicial de sobrescrever `window.snapshot.
  reply` pra um no-op foi descartada por não funcionar — `contextBridge`
  ignora silenciosamente essa reatribuição). Um `debug:listener-count`
  novo (mesmo guard `!app.isPackaged` de `chat:test-simulate-tool`) prova
  `ipcMain.listenerCount(...)` real: 0 antes, 1 esperando, e 0 de novo
  depois do timeout — não só que a chamada eventualmente retorna.
  Regressão limpa em `smoke-snapshot.mjs`/`smoke-mcp-read-card.mjs`.
  `tsc --noEmit`/`electron-vite build` limpos.
- Detalhe completo em `DESIGN-BACKLOG.md` item 63, seção Bugs de lógica.

## 2026-08-30 — item 63: auditoria pré-release, B4 fechado

- **B4** — `runSandboxedBash` (`sandbox.ts`) concatenava toda saída de
  `stdout`/`stderr` num `out` sem teto, cortando pra `MAX_OUTPUT_CHARS`
  só no `close` do processo — um comando aprovado imprimindo dezenas de
  MB fazia o heap do próprio main crescer proporcionalmente por todo o
  budget de 60s do comando. `appendChunk` novo para de acumular assim
  que passa de `MAX_OUTPUT_CHARS` (um `slice`, depois todo chunk
  seguinte é descartado sem tocar `out` de novo) — o comando sandboxado
  continua rodando normal, só que main para de reter o que ele produz
  depois do corte.
- Verificado ao vivo, sem mock: novo `smoke-sandbox-bash-output-cap.mjs`
  (6 checks) — `bash` real aprovado via consentimento real rodando `yes |
  head -c 50000000` (50MB reais) dentro do bwrap; resultado volta
  truncado, e um novo `debug:heap-used-mb` mede o heap REAL do processo
  main: cresceu só 0.9MB capturando o comando de 50MB. Regressão limpa em
  `smoke-chat-sandbox.mjs`/`smoke-sandbox-home-occlusion.mjs`. `tsc
  --noEmit`/`electron-vite build` limpos.
- Detalhe completo em `DESIGN-BACKLOG.md` item 63, seção Bugs de lógica.

## 2026-08-30 — item 63: auditoria pré-release, B5 fechado

- **B5** — detecção de URL vista no output do PTY (`pty-registry.ts`)
  rodava sobre cada chunk CRU do `onData`, não sobre o buffer já
  coalescido que `flush` monta pro renderer — uma URL mais longa que um
  chunk típico de PTY ficava partida entre dois `onData`, perdida ou
  emitida cortada. Match movido pra dentro de `flush`, rodando sobre o
  texto já junto; um `urlCarry` novo (teto de 2048 caracteres) guarda a
  cauda ainda-podendo-continuar entre flushes pro caso mais raro de
  split bem na fronteira do flush.
- Verificado ao vivo, sem mock: novo `smoke-pty-url-chunk-split.mjs` (5
  checks) — bash real imprimindo uma URL de 8000 caracteres via
  `window.pty.write`: chega inteira e uma única vez no `pty:url-seen`
  real, e o chip real no DOM mostra a URL completa. Regressão limpa em
  `smoke-terminal-links-paste.mjs`/`smoke-card-wheel-scope.mjs`. `tsc
  --noEmit`/`electron-vite build` limpos.
- Detalhe completo em `DESIGN-BACKLOG.md` item 63, seção Bugs de lógica.

## 2026-08-30 — item 63: auditoria pré-release, B7/B9 fecham a seção de Bugs de lógica (B1-B7, B9 ✅ completos)

- **B7** — `seenUrls` (pty-registry.ts, um `Set` por terminal) crescia
  sem limite. `MAX_SEEN_URLS = 500` com eviction do mais antigo (ordem
  de inserção do próprio `Set`) quando passa do teto — a URL evictada só
  sai do dedupe, nunca deixa de ser reportada quando genuinamente nova.
  Verificado ao vivo: novo `smoke-pty-seen-urls-cap.mjs` (4 checks) — 800
  URLs distintas reais impressas num loop, todas chegam via
  `pty:url-seen`, e um novo `debug:seen-urls-count` confirma o `Set`
  interno travado em 500. Regressão limpa em
  `smoke-terminal-links-paste.mjs`/`smoke-pty-url-chunk-split.mjs`.
- **B9** — decisão, não bug: `listChatSessions` (`store.ts`) deliberadamente
  NÃO filtra `archived_at`, ao contrário das outras duas queries de
  `cards`. Confirmado intencional (a sidebar de sessões precisa listar
  arquivadas pra `unarchiveCard`/o badge "arquivada" de `ChatCard.tsx`
  fazerem sentido) — comentário no código reforçado explicando a
  assimetria, nada de comportamento mudou.
- `tsc --noEmit`/`electron-vite build` limpos em cada passo.
- Detalhe completo em `DESIGN-BACKLOG.md` item 63, seção Bugs de lógica —
  fecha B1 até B9 (o item 63 inteiro de Bugs de lógica).

## 2026-08-30 — item 63: auditoria pré-release, Performance P3 fechado

- **P3** — `store.ts` nunca setava journal mode nem tinha índice nas
  colunas de filtro mais comuns. `db.pragma("journal_mode = WAL")` logo
  na abertura; 5 índices novos (`idx_cards_board_id`,
  `idx_connectors_board_id`/`_from_card_id`/`_to_card_id`,
  `idx_tasks_board_id`). `tasks.deps_json` deliberadamente NÃO indexado
  — conferido que nenhuma query filtra por ele, um índice ali não
  ajudaria nada.
- Verificado ao vivo, sem mock: novo `smoke-store-wal-indexes.mjs` (6
  checks) — segunda conexão direta `better-sqlite3` no `.db` real de uma
  sessão real, `PRAGMA journal_mode` confirma `wal`, `EXPLAIN QUERY PLAN`
  de cada query confirma uso do índice certo, não um scan de tabela
  inteira. Regressão limpa em `smoke-boot.mjs`/`smoke-card-lifecycle.mjs`.
  `tsc --noEmit`/`electron-vite build` limpos.
- Detalhe completo em `DESIGN-BACKLOG.md` item 63, seção Performance.

## 2026-08-30 — item 63: auditoria pré-release, Performance P4 fechado

- **P4** — `secrets.ts`'s `has`/`get`/`getBaseURL` reliam e reparseavam
  `secrets.json` do disco a cada chamada (`chat:send` sozinho dispara
  2-3 por mensagem). Cache em memória populado na primeira leitura;
  `set`/`clear` escrevem o estado novo direto no cache (nunca invalidam
  e releem) — seguro porque este processo é o único escritor do
  arquivo.
- Verificado ao vivo, sem mock: novo `smoke-secrets-cache.mjs` (9
  checks) — seta uma key real, deleta `secrets.json` do disco por FORA
  do app, confirma que `hasKey` ainda responde certo (prova que veio do
  cache, não de uma releitura); um provider nunca configurado continua
  respondendo `false` depois da mesma deleção; uma escrita real depois
  recria o arquivo com as duas keys da sessão. Regressão limpa em
  `smoke-secrets-atomic-write.mjs`/`smoke-secrets-settings.mjs`. `tsc
  --noEmit`/`electron-vite build` limpos.
- Detalhe completo em `DESIGN-BACKLOG.md` item 63, seção Performance.

## 2026-08-30 — item 63: auditoria pré-release, Performance P2 fechado

- **P2** — cada `BrowserCard` pintava a 30fps fixo, focado ou não.
  `setFocused(id, focused)` novo (`browser-registry.ts`): 30fps só pro
  card no topo do z-order (reusa o `order` que `App.tsx` já mantém, sem
  estado novo — `zIndex === order.length - 1`), 8fps pra qualquer outro
  visível-mas-não-focado. Ortogonal ao `stopPainting()` de invisível já
  existente.
- Verificado ao vivo, sem mock: novo
  `smoke-browser-unfocused-throttle.mjs` (4 checks) — dois BrowserCards
  reais numa página local com `requestAnimationFrame` (repaint contínuo
  genuíno): o focado chegou com 75 frames em 2.5s (bate ~30fps), o
  não-focado com 20 (bate ~8fps). Regressão limpa em
  `smoke-browser.mjs`/`smoke-browser-fullscreen-crash.mjs` (uma falha
  rodando os dois em paralelo desapareceu isolado — limitação já
  conhecida da suíte, não uma regressão real). `tsc --noEmit`/
  `electron-vite build` limpos.
- Detalhe completo em `DESIGN-BACKLOG.md` item 63, seção Performance.

## 2026-08-30 — item 63: auditoria pré-release, P1 fechado (os 8 tipos de card)

- **P1** (zero memoização no renderer, o item mais invasivo da auditoria
  — decisão de escopo revisada com o usuário antes de começar): dois
  hooks novos, `useStableCardHandler`/`useStableCardIdHandler`
  (`useStableCardHandler.ts`), devolvem a MESMA referência de função pra
  um card entre renders (`WeakMap` chaveado pelo próprio objeto do
  card — limpo pelo GC sozinho, sem cache não-limitado). A closure
  cacheada sempre chama a versão MAIS RECENTE da função real via uma
  `ref` — achado real investigando: `onConnectorStart` fecha sobre
  `order`, um cache ingênuo sem esse `ref` teria travado numa ordem
  antiga de verdade. `TerminalCard` (o tipo mais numeroso/mais
  arrastado) foi o primeiro convertido; os outros 7 (FilesCard,
  ChangesCard, StickyCard, StrokeCard, RemoteWindowCard, ChatCard,
  BrowserCard) seguiram o MESMO padrão sem nenhuma descoberta nova —
  cada `onX={...}` inline em `App.tsx` virou `getXHandler(c)`, cada
  componente virou `XCardInner` + `export const XCard =
  memo(XCardInner)`. `ChatCard`/`BrowserCard` ganharam a mesma
  instrumentação de contador de render que `TerminalCard` (têm prop
  `id`); os outros 5 não recebem `id` de `App.tsx` — não valeu mudar a
  assinatura só pra instrumentação de teste, dependem da suíte de
  regressão funcional pra prova.
- Verificado ao vivo, sem mock: `smoke-render-memoization.mjs` (17
  checks) — contador de render real (`window.__cardRenderCounts`)
  contra dois terminais reais, um chat real e um navegador real:
  arrastar UM terminal re-renderiza só aquele, o irmão fica idêntico
  byte a byte; arrastar o chat re-renderiza só o chat; criar o
  navegador não re-renderiza chat nem terminais; panar o board inteiro
  não causa NENHUM render extra em nenhum dos quatro (antes, todos os
  cenários reconciliavam a árvore inteira a cada `pointermove`).
  **Achado real ao vivo, não é bug de memoização**: `tryChangeRect`
  rejeita qualquer movimento de card não-navegador que sobreponha um
  `BrowserCard` (`WebContentsView` nativa sempre pinta por cima —
  regra de produto real, não relacionada a este item); como o app
  posiciona cards novos quase totalmente sobrepostos em cascata,
  arrastar o chat DEPOIS de já existir um navegador cancelava o drag
  silenciosamente — corrigido no próprio teste (chat criado e arrastado
  antes do navegador existir), não no produto. Regressão limpa na
  suíte `verify` inteira (todos os `scripts/verify/smoke-*.mjs`,
  incluindo lifecycle, actions, wheel-scope, font-zoom, links-paste,
  visibility-persist, read-card, connector, group-select, files-card,
  radial-longpress, constellation, chat-tools, chat-sandbox, browser).
  `tsc --noEmit`/`electron-vite build` limpos.
- Achado real ao vivo verificando (não do harness, corrigido no próprio
  teste): um `\r` sozinho no chunk de PTY parou de provar com
  segurança "Enter foi apertado" em `smoke-terminal-install-hint.mjs` —
  o timing do `memo` novo faz um resize do PTY redesenhar a linha ainda
  não submetida (confirmado sem saída real do `npm` nem prompt novo no
  meio). Critério do teste endurecido pro que ele realmente quer
  provar.
- Depois de fechar P1, a suíte `verify` inteira (75 scripts) rodou uma
  vez do início ao fim contra o build final. Três achados incidentais,
  nenhum é regressão de memoização: **(1)** `smoke-browser.mjs` teve o
  flake de canvas-size já documentado (concorrência de instâncias
  Electron), confirmado passando isolado. **(2)**
  `smoke-terminal-font-zoom.mjs` tem um flake pré-existente no seu
  próprio poll de canvas (já avisado no comentário do teste) —
  confirmado ao vivo que o fontSize sempre acompanha o zoom
  corretamente, é a técnica de poll que é frágil. **(3)**
  `smoke-terminal-visibility-persist.mjs` "travava" em reruns porque o
  teste reusa seu `.verify-tmp/` sem resetar o SQLite — depois de uma
  sessão criada com sucesso, a run seguinte pula a Home e quebra o
  próprio setup; com diretório limpo passa em ~10s. **Achado incidental
  fora do escopo de P1** (registrado como item novo na seção
  Organização/CI/docs do `DESIGN-BACKLOG.md`): `smoke-mcp-
  concurrency-cap.mjs` trava de verdade — espera REJEIÇÃO ao bater o
  cap, mas `autonomousSpawn` (item 60, sessão anterior) hoje ENFILEIRA
  nesse cenário; o teste nunca foi atualizado depois que a fila passou
  a existir.
- Detalhe completo em `DESIGN-BACKLOG.md` item 63, seção Performance —
  fecha o item 63 inteiro de Performance (P1-P4; P5 pulado por decisão
  do usuário).

## 2026-08-31 — item 65: 5 pontos do chatbox reportados ao vivo, 4/5 fechados (imagens/anexos vira item 66)

- **Ponto 1 — "+" nova sessão**: abria um SEGUNDO card em cascata em vez
  de resetar o atual. Perguntado ao usuário; escolhido resetar o card
  atual. `newChatSession(cardId, provider)` (App.tsx) agora arquiva a
  conversa velha sob o id antigo (sem perda — mesmo mecanismo de
  fechar um chat, item 30) e cria um card novo na MESMA posição/`order`,
  substituindo o antigo — visualmente é "o mesmo" card resetado.
  `smoke-chat-new-session-per-provider.mjs` reescrito (12 checks):
  board continua com 2 cards (não 3), card resetado tem id de linha
  novo + conversa vazia, sessão antiga sobrevive arquivada.
- **Ponto 2 — modelos gemini defasados**: usuário anexou print do
  painel de uso real mostrando só "Gemini 3.7 Flash" e "Antigravity"
  como categorias de quota ativas. `PROVIDER_MODELS.gemini`
  (secretsUi.ts) reduzido a `["gemini-3.7-flash"]`. "Antigravity" NÃO
  entrou — é a Interactions/Agents API do Gemini, forma de chamada
  diferente da Chat Completions que este app fala; incluir quebraria.
  `smoke-chat-providers.mjs` (já existente) confirma o default.
- **Ponto 3 — "invalid tool call index: undefined"**: achado real
  lendo o SDK `openai` — seu helper de streaming reconstrói tool_calls
  por `delta.tool_calls[].index` e joga um erro seco se algum endpoint
  "compatível" (gemini via este shim, confirmado pelo usuário) não
  manda esse campo — mesmo com a resposta real já processada/cobrada
  no servidor. `openai-client.ts` ganhou um fallback: esse erro
  específico dispara uma chamada NÃO-streaming da mesma requisição, sem
  parsing incremental pra quebrar nesse gap. Novo
  `smoke-chat-tool-call-index-fallback.mjs` (4 checks) reproduz a causa
  raiz exata (endpoint local que streama um tool_call sem `index`) e
  confirma o fallback renderiza a resposta em vez de travar em erro.
- **Ponto 4 — sem botão de parar inferência**: o botão de enviar só
  desabilitava; `window.chat.cancel` já existia (best-effort, aborta o
  stream) mas nunca tinha UI (o próprio comentário de
  `anthropic-client.ts` já registrava essa lacuna). Agora o mesmo botão
  vira um botão de parar (ícone `interrupt`, mesmo do Ctrl+C do
  TerminalCard) durante `streaming !== null`; o texto PARCIAL já
  gerado é commitado como mensagem final (não descartado). Novo
  `smoke-chat-stop-button.mjs` (9 checks) — endpoint local que streama
  devagar, confirma a conexão real fecha (não só a UI muda) e o texto
  parcial exato vira a mensagem salva.
- **Ponto 5 — imagens/anexos**: confirmado que não existe (`content` é
  string pura em todo o pipeline). Perguntado ao usuário implementar
  agora vs. registrar; escolhido registrar — vira item 66, não
  priorizado ainda.
- `tsc --noEmit`/`electron-vite build` limpos. Regressão limpa em
  `smoke-chat.mjs`/`smoke-chat-tools.mjs`/`smoke-chat-sandbox.mjs`/
  `smoke-chat-sessions-sidebar.mjs`/`smoke-render-memoization.mjs`.
- Detalhe completo em `DESIGN-BACKLOG.md` itens 65 e 66.

## 2026-08-31 — 4 pontos reportados ao vivo depois do item 65: copiar texto no terminal, rodapé colidindo com o resize, install-hint desatualizado, DESIGN-BACKLOG.md consolidado

- **Copiar texto no terminal** ("não consigo copiar textos"): xterm.js
  renderiza em canvas/WebGL — uma seleção visual nunca virava nada
  copiável, nenhum handler de cópia existia. `useTerminal.ts` ganhou
  Ctrl+Shift+C (não Ctrl+C sozinho, que continua reservado pro SIGINT
  — mesma convenção de GNOME Terminal/Konsole/xterm) escrevendo
  `term.getSelection()` na área de transferência real. Novo hook
  só-de-teste `window.__selectTerminalTextForTest` (terminal-
  registry.ts, mesmo espírito do `window.__cardRenderCounts` do P1) usa
  `term.select()` — a MESMA primitiva do drag do mouse — pra selecionar
  um marker real sem precisar de coordenadas de pixel sobre um canvas.
  Novo `smoke-terminal-copy.mjs` (3 checks): seleção real, Ctrl+Shift+C
  real via CDP escreve na área de transferência real do SO, e Ctrl+C
  SOZINHO confirmado como NÃO mexendo na área de transferência (SIGINT
  intacto).
- **Rodapé colidindo com a alça de resize** ("precisa de space-between,
  nenhum invadir espaço do outro"): screenshot ao vivo (drag-resize real
  até um card estreito) confirmou o badge de URL do rodapé quase
  encostando na alça de resize (22×22px, `right:0;bottom:0`, fora de
  `.card-clip` — sempre por CIMA do rodapé nesse canto). `.card-foot`
  (cards.css, base compartilhada por TODOS os tipos de card com
  footer) ganhou `padding-right: 28px` (22px do grip + respiro) — fix
  de um lugar só, não por-tipo. Confirmado ao vivo com screenshot
  antes/depois via CDP.
- **`smoke-terminal-install-hint.mjs`/`smoke-provider-gemini.mjs`
  quebrados por deriva de ambiente, não regressão**: os 4 providers
  terminal-spawnáveis (claude/codex/cursor/gemini) passaram a estar
  TODOS instalados nesta máquina desde a última verificação — a premissa
  dos dois testes ("gemini genuinamente ausente") deixou de ser
  verdade. Fix nos dois testes (não no produto): o diretório real do
  binário é removido do `PATH` do processo Electron FILHO antes do
  `startApp` (`which()` real, `providers.ts`, resolve por
  `process.env.PATH`) e restaurado logo depois — o app enxerga
  "binary_not_found" de verdade sem inventar nenhum provider fake nem
  tocar em código de produto.
- **Achado ao vivo, ainda sem ação**: `which antigravity` confirma que
  esse binário NÃO existe nesta máquina — o pedido do usuário ("mude o
  gemini para o antigravity CLI") provavelmente é sobre trocar/adicionar
  esse provider terminal-spawnável, mas o escopo exato (substituir
  gemini? adicionar como 5º provider?) ainda não foi confirmado.
  Registrado em `DESIGN-BACKLOG.md`, seção "Em Andamento", junto com o
  pedido de mascarar o path de imagem colada no terminal (também
  aguardando esclarecimento).
- **`DESIGN-BACKLOG.md` foi consolidado por outra sessão/processo
  durante este turno** — de um log cronológico de ~6700 linhas (itens
  numerados 1-66) pra um resumo vivo por categoria (~150 linhas: Em
  Andamento, Pendente, Ideias, Concluído). O conteúdo dos itens 65/66
  desta sessão sobreviveu, incorporado nas novas categorias. Daqui pra
  frente, edições nesse arquivo devem seguir o formato novo (bullets
  curtos por categoria) — o detalhe cronológico completo continua
  vivendo aqui no `AGENTS.md` (ainda não migrado pra `docs/HISTORY.md`,
  ver `DESIGN-BACKLOG.md` seção 2.3).
- `tsc --noEmit`/`electron-vite build` limpos. Regressão limpa em
  `smoke-card-wheel-scope.mjs` (flake de contenção de processo,
  confirmado limpo isolado, não regressão real). Achado adicional em
  `smoke-card-actions.mjs` (não corrigido, fragilidade do PRÓPRIO
  teste): seu drag de pan termina num `mouseReleased` em coordenadas
  negativas/fora da viewport (`-1500,-1500`) — fisicamente impossível
  pra um mouse real — e isso deixa o clique SEGUINTE no rail quebrado
  (popover não abre); confirmado ao vivo que um pan com destino
  DENTRO da viewport não reproduz nada disso. Não relacionado a
  nenhuma mudança desta sessão.

## 2026-08-31 — Provider Gemini → Antigravity CLI e máscara visual de path de imagem colada, resolvendo os dois pontos deixados em aberto acima

- **Esclarecimentos do usuário (AskUserQuestion)**: "mude o gemini para o
  antigravity CLI" → **substituir** o provider terminal-spawnável
  `gemini` (`ProviderId`, `providers.ts`) por `antigravity`, em todo
  lugar que o lista — não tocar no `ChatProvider` "gemini" separado, que
  é a integração de API do chatbox e continua existindo. "Máscara visual
  do path de imagem colada" → **só a aparência muda**: o terminal
  DISPLAY um placeholder curto (`[imagem #N]`) em vez do path cru; os
  bytes reais enviados/lidos pela CLI que estiver rodando no PTY
  continuam absolutamente intocados — a CLI nunca percebe diferença
  nenhuma.
- **Pesquisa real antes de codar** (WebSearch + WebFetch, 2 fontes
  cruzadas): Google aposentou o Gemini CLI e o substituiu pelo
  Antigravity CLI, um agente de terminal em Go (binário `agy`) que
  compartilha o motor com o app desktop Antigravity 2.0. Instalação:
  `curl -fsSL https://antigravity.google/cli/install.sh | bash`. Flags:
  `--model`, `--effort low|medium|high`, `--conversation <id>`,
  `--continue`/`-c`, `-p`/`--print`, `--output-format json`. Confirmado
  ao vivo nesta máquina depois (`agy` já está instalado aqui,
  `agy --version` → 1.1.22): `agy -p "..." --output-format json` usa a
  chave `"response"` (não `"result"`) — `extractJsonResult`
  (`ai-action.ts`) já cobria os dois casos, sem mudança de código
  necessária, só o comentário deixou de ser uma suposição.
- **Rename mecânico, com cuidado pra não confundir os dois "gemini"**:
  `providers.ts` (`ProviderId`, entrada `PROVIDERS`), `mcp-server.ts`
  (enum Zod do `spawn_agent`), `chat-tools.ts` (`DelegateProvider`, enum
  da ferramenta de delegação), `App.tsx` (`PROVIDER_OPTIONS` do popover
  de criação de terminal), `ProviderPicker.tsx` (`PROVIDER_ICON`),
  `icons.tsx` (`providerGemini` → `providerAntigravity`, ícone novo
  `Rocket` do lucide-react), `FilesCard.tsx`/`ai-action.ts`/
  `session-watch.ts` (comentários). Deixados intocados, confirmados um
  por um: `secretsUi.ts` (`PROVIDER_MODELS.gemini`, modelos de chat),
  `ChatCard.tsx`/`App.tsx` (mapeamento de modelo default do
  `ChatProvider` "gemini"), `mcp-server.ts`/`chat-tools.ts` (qualquer
  referência ao `ChatProvider` separado).
- **Modelos Gemini do chatbox também corrigidos** (ponto 2 do pedido ao
  vivo original, resolvido antes deste rename): screenshot do painel de
  uso do usuário confirmou que a família 2.5 (`gemini-2.5-flash`,
  `gemini-2.5-pro`, `gemini-2.5-flash-lite`) está desativada nessa
  conta — `PROVIDER_MODELS.gemini` (`secretsUi.ts`) ficou só com
  `gemini-3.7-flash`. "Antigravity" aparece no MESMO painel de uso mas
  deliberadamente NÃO foi adicionado à lista de modelos de chat: é a API
  de Interactions/Agents, formato de chamada diferente do Chat
  Completions que o pipeline de chat deste app fala.
- **Máscara visual do path de imagem** (`useTerminal.ts`): novo
  `pendingMaskRef`/`maskBufferRef`/`writeMasked()` — o PTY local-echo
  (o próprio processo remoto ecoa de volta o que recebeu, comportamento
  padrão de TTY) intercepta SÓ o que UM listener específico de
  `pty:data` passa pra `term.write()`; o path real continua sendo
  escrito no PTY via `window.pty.write` sem alteração nenhuma —
  provado no teste novo com um SEGUNDO listener de `pty:data`,
  independente do interno do app, que capturou o path bruto completo.
  Degrada com segurança: se o buffer crescer além do tamanho do needle
  sem casar (ex.: uma CLI baseada em Ink redesenhando com códigos ANSI
  no meio), descarta a máscara e libera o buffer cru — nunca arrisca
  perder ou corromper output real do terminal.
- **`smoke-terminal-image-mask.mjs` (novo, 8 checks)**: reusa a técnica
  de paste sintético de imagem já usada por `smoke-terminal-links-
  paste.mjs` (`ClipboardEvent('paste')` com `DataTransfer` contendo um
  `File` `image/png`). Achado ao investigar a primeira falha: o próprio
  handler do app chama `clipboardImage.save()` de novo internamente ao
  processar o paste, gerando SEU PRÓPRIO arquivo/timestamp — diferente
  de um path obtido chamando `save()` do lado do teste antes do paste.
  Fix: o teste extrai o path real a partir do que o SEGUNDO listener de
  `pty:data` efetivamente capturou (fonte da verdade), confirma que o
  arquivo existe de fato em disco, e só então checa que a TELA
  renderizada (via `read_card` MCP) mostra `[imagem #1]`/`[imagem #2]`
  (dois pastes seguidos, contador incrementando) e nunca o path cru.
- **`smoke-provider-gemini.mjs` renomeado para `smoke-provider-
  antigravity.mjs`** e **`smoke-terminal-install-hint.mjs` reescrito**
  pro novo provider: `agy` está genuinamente instalado nesta máquina
  agora (mesma deriva de ambiente do achado anterior), então a técnica
  de raspar o diretório real do binário do `PATH` do processo Electron
  filho (antes de `startApp`, restaurado logo depois) continua sendo
  necessária — só trocando o alvo de `gemini` pra `agy`/`antigravity`
  em ambos. Comando de instalação no tooltip/PTY também atualizado pro
  comando real do Antigravity.
- `tsc --noEmit`/`electron-vite build` limpos. Regressão completa:
  `smoke-terminal-links-paste.mjs` (19 checks), `smoke-chat-new-
  session-per-provider.mjs`, `smoke-chat-stop-button.mjs`, `smoke-chat-
  tool-call-index-fallback.mjs`, `smoke-terminal-copy.mjs` (3 checks),
  `smoke-chat-providers.mjs` (confirma o `ChatProvider` "gemini" do
  chatbox intacto) — todos limpos isolados. Duas falhas observadas rodando
  6 testes em sequência (paste de imagem e clipboard "Document is not
  focused") reproduziram-se como flake de contenção de recursos
  (janelas Electron concorrentes disputando foco/clipboard do SO), não
  regressão real — confirmado limpo ao isolar cada um.
- Detalhe completo em `DESIGN-BACKLOG.md`, seções 4.2 (Terminal
  Avançado) — os dois itens de "Em Andamento" deixados abertos na
  entrada anterior foram removidos de lá e incorporados aqui.

## 2026-08-31 — Piscar/flicker no arraste de cards (item 2.1, primeiro de uma lista priorizada de itens pendentes)

- **Contexto**: usuário pediu uma lista priorizada dos itens mais difíceis
  do backlog pendente (`DESIGN-BACKLOG.md` seção 2.1) e a ordem
  recomendada de execução; aprovou a lista com uma ressalva — outro
  agente já cuida do FilesCard, então esse item fica de fora. Ordem
  aprovada: (1) flicker no terminal, (2) chatbox imagens/anexos, (3)
  ~~FilesCard~~, (4) exportação de canvas, (5) mídias Miro/Figma.
- **Causa raiz encontrada lendo o código, sem screenshot** (captura de
  canvas/WebGL não é confiável neste ambiente, ver AGENTS.md):
  `CardFrame.tsx`'s `onMove` (drag do header E do resize) chamava
  `onChange(rect)` direto dentro do listener nativo de `pointermove`, uma
  vez por evento CRU, sem nenhum agrupamento — e `onChange` sobe até
  `App.tsx`'s `changeRect`/`setCards`, que muda `left`/`top` (propriedades
  de LAYOUT, não só composição) no estilo inline do card. `pointermove`
  pode chegar bem mais rápido que a taxa real de atualização da tela —
  cada evento virava seu próprio layout+paint forçado, sem coalescing
  nenhum com o pipeline de renderização do browser. Conteúdo DOM comum
  (CodeMirror, markdown, notas) absorve esse trabalho redundante sem
  sintoma visível; o canvas WebGL do xterm.js é exatamente o tipo de
  camada onde repaint/composite não-sincronizado aparece como piscar —
  por isso só o terminal foi reportado, mesmo o bug existindo em
  `CardFrame` (compartilhado por todos os 8 tipos de card).
  GPU acceleration está reabilitada nesta máquina desde 2026-08-26 (item
  9), então a hipótese descartada foi "WebGL caindo pra software
  rendering" — a causa real é puramente sobre CADÊNCIA de atualização,
  não sobre o pipeline gráfico em si.
- **Fix** (`CardFrame.tsx`, compartilhado — não uma mudança
  terminal-específica): `rafThrottleRect`, um agrupador local por gesto
  de arraste/resize, junta múltiplos `pointermove` crus no mesmo
  `requestAnimationFrame` num único `onChange`. No `pointerup`, um
  `flushAndCancel` cancela qualquer rAF pendente e aplica o `finalRect`
  de forma síncrona ANTES de `onCommit` — garante que a posição visual
  final bate exatamente com o resultado persistido no banco, sem
  depender do último rAF ter chegado a disparar. Não muda a lógica do
  arraste em si (matemática de `dx`/`dy`, sincronia de grupo em
  `changeRect`) — só a cadência de quando `onChange` é chamado.
- **`smoke-card-drag-raf-throttle.mjs` (novo, 6 checks)**: prova
  quantitativa, não visual. Dispara uma rajada de 40 `mouseMoved`
  sintéticos via CDP SEM aguardar a resposta de cada um individualmente
  (só `Promise.all` no fim) — um round-trip de CDP sozinho já costuma
  passar de 16ms, o que mascararia o throttle sendo testado se cada
  evento esperasse sua resposta. Resultado real: 40 eventos crus → **1
  único render** (via `window.__cardRenderCounts`, mesmo hook do P1).
  Confirma também que a posição final na TELA (`getBoundingClientRect`)
  e o rect persistido no banco (`window.store.list`, que retorna
  `CardRow` com colunas planas `x`/`y`, não um `rect` aninhado — achado
  ao escrever o teste) batem exatamente com o deslocamento total do
  drag — agrupar frames não perde nem atrasa o resultado real.
- **Regressão**: `smoke-card-actions.mjs` (pan/focus, 6 checks) e
  `smoke-terminal-visibility-persist.mjs` (3 checks) limpos. Achado
  colateral, não relacionado a esta mudança: `smoke-group-select.mjs`,
  `smoke-render-memoization.mjs` e `smoke-connector.mjs` falham hoje
  num seletor stale (`.rail-btn[title="Nova nota adesiva"]`/"Novo
  chatbox"/"Novo navegador") — a Rail foi consolidada num popover único
  "Adicionar card" (`data-kind` nos itens) em algum momento não coberto
  por esses 3 testes; `smoke-provider-antigravity.mjs` e
  `smoke-terminal-install-hint.mjs` (escritos nesta mesma sessão, ver
  entrada anterior) já receberam um fallback pra esse popover — indício
  de que outra sessão está ativamente corrigindo essa classe de
  seletores em paralelo, então os 3 arquivos acima foram deixados
  intocados de propósito (evitar conflito de edição), não corrigidos
  aqui. Interação `connector`/`select` usa um caminho de pointer
  totalmente separado (`onConnectorStart`/`onSelectStart`, nunca toca
  `onHeaderPointerDown`/`onResizePointerDown`) — não afetado por esta
  mudança independentemente do achado acima.
- `tsc --noEmit`/`electron-vite build` limpos.
- Próximo da lista aprovada: chatbox — suporte a imagens/anexos no
  composer (Itens 65.5/66, `DESIGN-BACKLOG.md` 2.1).

## 2026-08-31 — Chatbox: anexos de imagem no composer (item 2 da lista priorizada)

- **Decisão de arquitetura** (não pedida explicitamente pelo usuário — a
  lista aprovada só dizia "implemente", sem detalhar formato de
  armazenamento — decidida com base no precedente já estabelecido no
  próprio app): `ChatMessage.content` vira `string | ChatContentBlock[]`,
  mas o bloco de imagem guarda só um **path em disco** (mesmo diretório
  `stellar-pastes` que `clipboard-image.ts`/`useTerminal.ts` já usam pro
  terminal), NUNCA base64 persistido em `messages_json`. Razão: essa
  coluna é TEXT puro no SQLite, reparseada inteira a cada render da lista
  de mensagens — base64 infla ~1.33x e ficaria pra sempre no banco, até
  depois da imagem sumir do composer. O main process só lê o arquivo e
  converte pra base64 na hora exata de montar a request de verdade
  (`anthropic-client.ts`/`openai-client.ts`), igual o terminal já faz com
  o path que escreve no PTY.
- **Type duplicado em 3 lugares** (padrão já estabelecido nesta base —
  main/renderer/preload são bundles TS separados, sem import cruzado):
  `card-types.ts` (renderer), `chat-tools.ts` (main), `preload/index.ts`
  — os três precisaram do mesmo `ChatContentBlock`/`ChatImageBlock`.
  `mediaType` restrito aos 4 formatos que a Anthropic aceita nativamente
  (`Base64ImageSource`: jpeg/png/gif/webp) — interseção segura com o que
  OpenAI/Gemini/custom também aceitam via `image_url` com data URI.
- **Paste E drop sem tocar no clipboard do SO**: diferente do terminal
  (xterm.js não expõe `File`, só texto — por isso o terminal precisa da
  API `clipboard` do Electron), o composer do chatbox é uma `<textarea>`
  comum — `paste`/`drop` já entregam um `File` de verdade
  (`clipboardData.items`/`dataTransfer.files`). `addImageFile` (único
  código para os dois gestos) lê via `FileReader.readAsDataURL`, valida o
  tipo contra os 4 aceitos, manda os bytes em base64 pro main
  (`clipboardImage.saveBytes`, IPC nova `chat:save-attachment-image`,
  com limite de ~10MB) — que grava em disco e devolve o path real.
  `clipboard-image.ts` ganhou `writeImageBuffer` (extraído de
  `saveClipboardImage`, reusado pelas duas gravações) e
  `saveImageBytes`/`readAttachmentImage`.
- **Miniatura de mensagem já enviada** (inclusive sessão restaurada, sem
  o preview em memória do composer): nova IPC `chat:read-attachment-image`
  relê o arquivo do disco sob demanda — validação de path escopada ao
  diretório `stellar-pastes` fixo (checa com o separador de path incluso,
  não um prefixo de string solto — um `stellar-pastes-outra-coisa/`
  passaria despercebido sem isso), mesma preocupação de segurança que
  `confine()` (fs-tools.ts) mas pra um diretório fixo em vez de um root
  por-card. Componente `ChatImageThumb` (ChatCard.tsx) com cache em
  memória por `path`, module-level — a mesma imagem reaparece em toda
  re-renderização da lista sem round-trip de IPC repetido.
- **Mapeamento por provider**: `toAnthropicMessages` (anthropic-client.ts)
  → `{type:"image", source:{type:"base64", media_type, data}}`;
  `toOpenAiMessages` (openai-client.ts, também usado por gemini/custom —
  ambos reusam o cliente OpenAI-compatible) → `{type:"image_url",
  image_url:{url: "data:<mediaType>;base64,<data>"}}`. Um arquivo que
  sumiu (limpeza de temp do SO) degrada pra um bloco de texto avisando em
  vez de derrubar o turno inteiro.
- **`sessionPreview`** (painel de sessões) ganhou `textOf()` — extrai só
  o texto de um `content` que agora também pode ser array; uma primeira
  mensagem só-de-imagem mostra "📎 imagem" em vez de quebrar/mostrar
  `[object Object]`.
- **`smoke-chat-image-attachment.mjs` (novo, 13 checks)**: servidor HTTP
  local real fazendo o papel de endpoint OpenAI-compatible (mesma técnica
  de `smoke-chat-providers.mjs`) — a prova real é que a REQUEST que ele
  recebe de verdade contém um `image_url` com um data URI base64 de
  fato preenchido, não um mock. Cobre: paste vira anexo pendente (preview,
  nada enviado ainda), drop de uma segunda imagem também funciona, remover
  um anexo pendente, envio real (texto + imagem) bate no endpoint com o
  content array correto, tira de anexos esvazia depois de enviar, e a
  própria bolha enviada mostra a miniatura relida de verdade via IPC.
- **Regressão**: `smoke-chat.mjs`, `smoke-chat-providers.mjs`,
  `smoke-chat-status-line.mjs` limpos (cobrem o caminho de texto puro,
  que continua funcionando sem tocar em nenhum bloco de imagem). Achado
  colateral, mesma causa dos 3 arquivos notados na entrada anterior (Rail
  restructurada num popover "Adicionar card"): `smoke-chat-tools.mjs`,
  `smoke-chat-sandbox.mjs`, `smoke-chat-write-consent-large-file.mjs`,
  `smoke-chat-consent-teardown.mjs` e `smoke-chat-sessions-sidebar.mjs`
  falham hoje num bug de outra sessão ainda no meio da correção desses
  seletores (`clickByTitle` com um `${JSON.stringify(title)}` escapado
  errado, virando `ReferenceError: title is not defined` dentro do
  browser) — deixados intocados de propósito, mesma razão de não
  conflitar edição já dada na entrada anterior; não relacionado a nenhuma
  mudança desta sessão.
- `tsc --noEmit`/`electron-vite build` limpos.
- Próximo da lista aprovada (item 3, FilesCard, fica de fora — outro
  agente já cuida dele): item 4, exportação de canvas com seleção de área
  (Item 57.8, `DESIGN-BACKLOG.md` 2.1).

## 2026-08-31 — Exportação do canvas com seleção de área (item 4 da lista priorizada)

- **Achado que decidiu a abordagem**: o app já tinha uma captura de janela
  real (`win.webContents.capturePage(rect)`) implementada pro `snapshot`
  MCP (`handleSnapshotRequest`, main/index.ts) — a mesma API que qualquer
  IA chamando `snapshot(rect)` já usa. Reusar essa API pro botão humano de
  "exportar" evita o problema clássico de bibliotecas DOM-to-canvas
  (html2canvas e afins): elas não conseguem capturar corretamente conteúdo
  WebGL (o terminal, `@xterm/addon-webgl`) nem `WebContentsView` nativas
  (o navegador embutido) — `capturePage` é uma captura real no nível do
  compositor, funciona pra qualquer coisa visível na janela.
- **Nova ferramenta "export"** (`Tool` — pointer/pen/connector/select
  ganham uma 5ª opção): arraste livre em coordenadas de TELA (client,
  não de mundo) — deliberadamente mais simples que o marquee de seleção
  de cards (`useCardSelection`, que vive em espaço de mundo transformado
  por pan/zoom): já é exatamente o espaço que `capturePage(rect)` espera,
  zero conversão necessária. Só pode começar em fundo vazio
  (`onBackgroundPointerDown`'s guard existente), mas o retângulo pode
  crescer POR CIMA de qualquer card durante o arraste — "recorte
  arbitrário", não uma seleção de cards.
- **Escondendo a própria UI antes de capturar**: a barra de formato
  (PNG/JPEG/PDF) e o retângulo tracejado são elementos DOM reais — se
  ainda visíveis no momento do `capturePage`, apareceriam DENTRO do
  próprio recorte exportado. `runExportSelection` esconde o overlay
  (`setExportSelection(null)`) e aguarda um duplo `requestAnimationFrame`
  (garante que o repaint sem o overlay já aconteceu — um commit de
  estado do React não é síncrono com o próximo paint do browser) ANTES
  de chamar a captura de verdade.
- **PDF sem lib nova**: um raster dentro de UMA página PDF é simples o
  bastante (PDF aceita JPEG embutido DIRETO como XObject
  `/Filter /DCTDecode`, sem re-comprimir) pra não justificar puxar
  jsPDF/pdf-lib. `main/pdf-export.ts`'s `wrapJpegAsPdf` monta os 5
  objetos PDF mínimos (catálogo/páginas/página/imagem/conteúdo) com
  offsets de xref calculados byte-a-byte. Decisão consciente antes de
  escrever qualquer linha: só valia implementar PDF se desse pra
  verificar de verdade que o arquivo gerado abre/rasteriza (não só "os
  bytes iniciais parecem certos") — `pdftoppm`/`pdfinfo` (poppler-utils)
  confirmados disponíveis nesta máquina antes de prosseguir; sem eles, o
  formato ficaria de fora desta rodada em vez de shipar sem prova real.
- **Limite honesto de teste, aceito conscientemente**: um diálogo nativo
  `dialog.showSaveDialog` não é dirigível via CDP (não é conteúdo web) —
  mesma limitação que `fs:pick-directory` já tinha, nunca testado ao
  vivo. Fix: `performExport` (main/index.ts) foi extraído do handler real
  pra ser reusável por uma segunda IPC test-only,
  `export:capture-rect-test` (`window.canvasExport.captureRectTest`,
  inerte em build empacotado — mesmo guard/precedente de
  `clipboard:test-write-image`), que pula SÓ o diálogo, rodando o mesmo
  código de captura/encode/escrita. O fluxo de UI real (ferramenta →
  arraste → barra de formato) é testado com eventos de mouse sintéticos
  de verdade; o pipeline de captura em si (a parte nova/arriscada) é
  testado via essa ponte.
- **`smoke-canvas-export.mjs` (novo, 22 checks)**: cobre a ferramenta
  ativando, arraste real down→move→up (com o card padrão quase
  preenchendo a janela no boot — achado ao vivo escrevendo o teste,
  precisou calcular a área de arraste a partir do rect real do card em
  vez de coordenadas fixas chutadas), retângulo aparecendo SEM a barra
  durante o arraste e COM ela depois de soltar, dimensões do recorte
  batendo exatamente com o arraste, cancelar limpando sem exportar nada,
  e os 3 formatos via `captureRectTest`: assinaturas de arquivo reais
  (PNG/JPEG/`%PDF`), tamanho do PNG substancial o bastante pra provar
  conteúdo real (não uma área vazia comprimida a quase nada — achado ao
  vivo calibrando o threshold: um recorte 400×200 real ainda compacta
  pra ~1KB, PNG é ótimo com áreas de cor sólida), e o PDF verificado de
  ponta a ponta com `pdfinfo` (dimensões da página batendo) e
  `pdftoppm` (rasteriza de volta pra um PNG real, não só "declara
  válido").
- **Escopo deliberadamente deixado de fora**: a ferramenta não foi
  adicionada ao menu radial (`RadialMenu.tsx`) — sua geometria de
  raio/ângulo é compartilhada por todos os itens da lista `ACTIONS`, e
  adicionar um 5º item de ferramenta ali não estava no pedido original,
  só no rail. Regressão limpa: `smoke-card-actions.mjs`,
  `smoke-radial-longpress.mjs`, `smoke-terminal-visibility-persist.mjs`
  (timeout observado rodando em lote, confirmado limpo isolado — mesmo
  padrão de flake de contenção já visto antes nesta sessão).
  `smoke-connector.mjs` segue com o mesmo bug de seletor de outra sessão
  já notado nas duas entradas anteriores, não relacionado.
- `tsc --noEmit`/`electron-vite build` limpos.
- Item 3 da lista aprovada (FilesCard) segue de fora — outro agente
  cuida dele. Item 5 (mídias no canvas estilo Miro/Figma) é o maior e
  mais mal-definido dos cinco — introduz um tipo de card inteiramente
  novo (visualizador de imagem/PDF com resize+aspect-ratio, rotação, zoom
  interno), decisões de design reais (ex.: qual lib de PDF viewer) que
  merecem o mesmo tipo de checagem prévia que os itens anteriores
  tiveram — reportado ao usuário antes de começar, em vez de seguir
  direto.
