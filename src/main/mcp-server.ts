...
export function createMcpServer(opts: { port: number; handleRequest: (req: BusRequest) => Promise<BusResponse> }) {
  ...
  function buildServer(urlCardId?: string): McpServer {
    ...
    server.registerTool(
      ...
      {
        description:
          "Read one task's current record by id — also includes its full status-transition trail (`transitions`), every card linked to it with a role (`cards`, e.g. one implementing + one reviewing, each with the recorded execution profile provider/model/effort and session identity requestedResumeId/sessionId when known — use sessionId ?? requestedResumeId as spawn_agent resumeId to reopen THAT participation's session after the card closed), contract fields (territory/gates/allowCommit/reportSchema), and its append-only verdict history (`verdicts`: one entry per participation round, `{cardId, role, verdict, at}`, `verdict: null` meaning that round ended without one) — unlike list_tasks which stays lean. Read-only: no tool writes to this history directly, it's derived from `report` calls and unreported exits. sessionId is null for providers that cannot resume (bash, antigravity, …) — honest absence, not a fake id.",
        inputSchema: {
          taskId: z.string().describe("The task's id (from create_task or list_tasks)"),
        },
      },
