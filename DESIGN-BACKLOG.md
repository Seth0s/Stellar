...
* **Navegador Embutido — Infraestrutura Chromium/Electron Madura:**
  ...
  * [ ] Navbar melhorada (histórico, favoritos) — parcialmente coberto pelo header abaixo (badge de origem clicável, presets de viewport), mas histórico/favoritos em si continuam de fora.
  ...
  * [ ] Header do card no mesmo padrão visual/funcional do navegador do CentralByte — parcialmente feito: badge de origem agora clicável (pan/raise até o card que abriu este navegador, via `jumpToCard`), badge de erro/aviso de console real (`webContents.on("console-message")`, contagem real, nunca existia captura nenhuma antes), presets de viewport (Mobile 390×844 / Tablet 768×1024, reusando o mesmo caminho `onChange`/`onCommit` de um resize por arraste) e o botão de DevTools acima. **Continua de fora** (arquitetura nova, registrado em §3): Design Mode (picker de elemento → enviar pro chat conectado), conectores com comportamento em tempo de execução, drawer de Rede/Scripts via `webContents.debugger`, favoritos/bookmark bar, header responsivo por breakpoint.
...
* **Header do Navegador — Restante da Paridade com CentralByte (§2.1, deliberadamente fora do escopo do plano de controle MCP + Trilha A):**
  ...
  * **Favoritos/bookmark bar** — precisa de schema de persistência novo (por board? global?), decisão de produto própria.
  * **Header responsivo por breakpoint de largura** — melhor decidir os breakpoints DEPOIS que Design Mode/drawer definirem o conjunto final de botões, senão redesenha duas vezes.
