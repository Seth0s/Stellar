...
* **Reabrir sessão anterior trava o app, principalmente sessão de agente com histórico grande — investigado ao vivo (2026-09-02), causa raiz NÃO confirmada, um gap real corrigido:**
  * Usuário refinou o relato: card COUNT não é o gatilho ("mesmo 1 é suficiente") — é sessão de agente com histórico grande.
  * Benchmarks reais via CDP (não simulados) descartaram 3 hipóteses: (1) até 75 cards (60 terminais bash + 15 navegadores) só produz um soluço de ~0.5-0.7s, recupera rápido; (2) burst de 4MB de texto cru por um PTY real não trava o pipeline `pty:data`→xterm.js (WebGL absorve sem hitch > 20ms); (3) `--resume` de uma sessão `claude` REAL de 56MB nesta máquina — a render thread do Stellar respondeu round-trips CDP normalmente o tempo todo (confirmado, não é main-thread block nem trava de GPU/compositor detectável por esse método).
  * *Achado real corrigido*: não existia NENHUM feedback visual entre "PTY spawnado" e "primeiro byte de output" — um terminal em silêncio (por qualquer motivo, curto ou longo) era visualmente idêntico a um card travado. Corrigido: `useTerminal.ts` ganhou `hasReceivedOutput` (true no primeiro `pty:data`), `TerminalCard.tsx` mostra um badge "carregando sessão…" debounced em 1200ms (não pisca num spawn normal) enquanto `ptyId` existe e nada chegou ainda. `smoke-terminal-loading-hint.mjs` novo cobre o caso que precisa ser sólido (spawn normal nunca mostra o aviso).
  * *Limitação honesta*: não consegui reproduzir remotamente um freeze de verdade em nenhum cenário testado — minha medição inicial ("60s sem output") foi descartada depois (seletor de DOM errado, xterm renderiza em WebGL/canvas, não tem `.xterm-rows`; a métrica real via `hasReceivedOutput` mostrou que o output chega bem antes de 1.2s mesmo nessa sessão de 56MB). Ou é algo que só se manifesta na janela REAL do usuário (GPU/driver/compositor Wayland-GNOME, que este AGENTS.md já registra como fonte de conflito noutro item) e não aparece via CDP numa instância isolada, ou é mais raro/específico do que "qualquer sessão grande".
  * *Pendente do usuário*: da próxima vez que acontecer — (a) trava só aquele card ou clicar/arrastar OUTRO card também não responde? (b) quanto tempo dura, aproximadamente? (c) acontece logo que a janela ganha foco/fica visível de novo? Essas 3 respostas decidem se a próxima investigação é main-thread (Stellar) ou GPU/compositor (SO).
  * *Achado colateral*: `smoke-terminal-visibility-persist.mjs` já é flaky no baseline (sem relação com esta mudança — confirmado rodando 4x contra o código antes desta correção, falhou nas 4). Registrado aqui pra não ser confundido com regressão desta mudança; investigar a parte de "escrita nova depois do ciclo" separadamente.
