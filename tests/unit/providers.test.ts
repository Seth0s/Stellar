...
describe("providers: impose session id (claude/cursor)", () => {
  ...
  // (`shippedProviderSpecs()`) nunca eram verificados. Foi por esse buraco
  ...
  it("nenhum spec DINAMICO impoe sessao — o conjunto que a assercao acima nao percorria", () => {
    expect(shippedProviderSpecs().length).toBeGreaterThan(1);
    for (const spec of shippedProviderSpecs()) {
      expect(
